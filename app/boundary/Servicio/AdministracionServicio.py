...
class AdministracionServicio:
    ...
    def initialize_consulta(self):
        ...
        self.tree = ttk.Treeview(
            self.frame_lista,
            columns=("ID", "Fecha", "Auto VIN",
                     "Tipo Servicio", "Costo", "Vendedor ID"),
            show="headings",
        )
        ...
        label_titulo = ctk.CTkLabel(
            self.frame_lista, text="Servicios por Auto", font=("Arial", 14, "bold"))
    ...
    def initialize_registro(self):
        ...
        self.entry_fecha = DateEntry(
            self.frame_registro, date_pattern='yyyy-mm-dd')
        self.entry_fecha.grid(row=0, column=1, padx=10, pady=10)

        self.label_auto_vin = ctk.CTkLabel(
            self.frame_registro, text="Auto VIN:")
        self.label_auto_vin.grid(row=1, column=0, padx=10, pady=10, sticky="w")
        self.entry_auto_vin = ttk.Combobox(self.frame_registro)
        # Llenar con la lista de VINs
        self.entry_auto_vin['values'] = self.obtener_lista_autos_vendidos()
        self.entry_auto_vin.grid(row=1, column=1, padx=5, pady=5, sticky="ew")

        self.label_tipo_servicio = ctk.CTkLabel(
            self.frame_registro, text="Tipo de Servicio:")
        self.label_tipo_servicio.grid(
            row=2, column=0, padx=10, pady=10, sticky="w")
        self.entry_tipo_servicio = ttk.Combobox(self.frame_registro)
        self.entry_tipo_servicio['values'] = [
            "Mantenimiento", "Reparación"]  # Opciones de la lista
        self.entry_tipo_servicio.grid(
        ...
        self.label_vendedor = ctk.CTkLabel(
            self.frame_registro, text="Vendedor:")
        ...
        self.boton_registrar = ctk.CTkButton(
            self.frame_registro, text="Registrar Servicio", command=self.registrar_servicio)
        self.boton_registrar.grid(
            row=5, column=0, columnspan=2, padx=10, pady=20)

        # Campos de consulta (VIN y botón)
        self.label_auto_vin_consulta = ctk.CTkLabel(
            self.frame_consulta, text="Ingrese VIN del Auto:")
        self.label_auto_vin_consulta.pack(side="top", padx=10, pady=10)

        self.entry_auto_vin_consulta = ttk.Combobox(self.frame_consulta)
        # Llenar con la lista de VINs
        self.entry_auto_vin_consulta['values'] = self.listar_autos()
        self.entry_auto_vin_consulta.pack(side="top", padx=10, pady=10)

        self.boton_consultar = ctk.CTkButton(
            self.frame_consulta, text="Consultar Servicios", command=self.consultar_servicios)
    ...
    def registrar_servicio(self):
        ...
        self.gestor_servicio.registrar_servicio(
            costo, auto_vin, tipo_servicio, vendedor, fecha)
        self.rellenar_tabla()
    ...
    def listar_servicios(self):
        ...
        for servicio in self.servicios.values():
            if isinstance(servicio, Servicio):
                tipo_servicio = self.gestor_tipo_servicio.obtener_tipo_servicio(
                    servicio.tipo_servicio_id)
                vendedor = self.gestor_vendedor.obtener_vendedor(
                    servicio.vendedor_id)

                tupla = (servicio.id, servicio.fecha, servicio.auto_vin,
                         tipo_servicio.nombre, servicio.costo, vendedor.nombre)
